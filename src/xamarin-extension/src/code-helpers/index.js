...
function generateXamarinRule(styleObj, projectColorMap, mixin) {
    ...
    return `<ResourceDictionary>${styleObjText}</ResourceDictionary>`;
}
...
function getStyleguideColorTexts(colorFormat, colors) {
    return colors.map(color => {
        ...
        return `  <Color x:Key="${color.name}">${colorStyleObject}</Color>"`;
    });
}
